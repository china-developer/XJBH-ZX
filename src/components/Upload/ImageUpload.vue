...
watch(
  () => props.modelValue,
  (newVal: string[]) => {
    const filePaths = fileList.value.map((file:any) => file.url);
    // 监听modelValue文件集合值未变化时，跳过赋值
    if (
      filePaths.length > 0 &&
      filePaths.length === newVal.length &&
      filePaths.every((x:any) => newVal.some((y) => y === x)) &&
      newVal.every((y) => filePaths.some((x:any) => x === y))
    ) {
      return;
    }
...
const handleSuccessFile = (response: any, file: UploadFile) => {
  if (response.code === ResultEnum.SUCCESS) {
    ElMessage.success(t('el.message6'));
    valFileList.value.push(response.data.url);
    emit("update:modelValue", valFileList.value);
    return;
  } else {
    ElMessage.error(response.msg || t('el.message7'));
  }
};

const handleError = (error: any) => {
  ElMessage.error(t('el.message7'));
};
...
function handleBeforeUpload(file: UploadRawFile) {
  if (file.size > props.uploadMaxSize) {
    ElMessage.warning(
      t('el.message8') + Math.trunc(props.uploadMaxSize / 1024 / 1024) + "M"
    );
    return false;
  }
  return true;
 ...
 * 预览图片
 */
const previewImg: any = (uploadFile: UploadFile) => {
  viewFileList.value = fileList.value.map((file:any) => file.url!);
  initialIndex.value = fileList.value.findIndex(
    (file:any) => file.url === uploadFile.url
  );
